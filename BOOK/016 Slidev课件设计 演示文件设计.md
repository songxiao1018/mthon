# 🎬 模板二·拓展：Slidev 课件演示文件设计（可直接渲染的 slides.md）

> **使用场景**：在 `015 Slidev 脚本安排设计.md` 已经把"每张幻灯片该长什么样"敲定之后，
...
> **配套 015 脚本**：`BOOK/015 Slidev课件设计 脚本安排设计.md`（本课实例）
...
012 教材草稿（按 x.0~x.4 五模块整合成完整 2 小时课）
   ↓
015 Slidev 脚本安排设计（按页拆分课件，锁定每页的版式 / 动画 / 用时 / 旁白）
   ↓
016 Slidev 演示文件设计（你在这里 —— 写真正能被 Slidev 渲染的 slides.md）
   ↓
